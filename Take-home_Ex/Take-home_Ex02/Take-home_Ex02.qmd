...
|------------------|------------------|-------------------|------------------|
| 1   | nián yú Ltd. Corporation         | Niger Bend Limited Liability Company Marine ecology | 2031: 3471, 
2030: 2979 |
| 2   | nián yú Ltd. Corporation         | Costa de la Felicidad Shipping                      | 2029: 2797, 
2028: 2473 |
...
Chhattisgarh Marine ecology A/S Delivery remains the top company with highest out-degree in the subsequent years except for 2034 where it is overtaken by Line S.A. de C.V. Although the number of outgoing links shown in the graph is only 4, this is due to the number of outgoing links <=10 being removed before plotting the graph. 
...
-   Due to the large data size, we filter data with betweenness \> 500 and community group \< 5. Group_component is used to group the nodes.
...
### 3.3.5 Taking a Closer Look at Caracola del Este Corporation Express

-   We want to see the fishing activities of Caracola del Este Corporation Express as it has one of the highest number of outgoing links but no incoming link.
...
MC2_edges_aggregated_odtop2028 <- MC2_edges_aggregated2028 %>%
  filter(source == "Selous Game Reserve  S.A. de C.V.") %>%
  arrange(desc(weights)) %>%
...
ggraph(MC2_graph_odtop2028, 
  ...
  theme_graph() +
  geom_node_text(aes(label = id), size = 1, repel=TRUE) +
  ggtitle("Activities of Selous Game Reserve S.A. de C.V. in 2028")
```
...
ggraph(MC2_graph_odtop2029, 
  ...
  theme_graph() +
  geom_node_text(aes(label = id), size = 1, repel=TRUE) +
  ggtitle("Activities of Caracola del Este Corporation Express in 2029")
```
...
ggraph(MC2_graph_odtop2030, 
  ...
  theme_graph() +
  geom_node_text(aes(label = id), size = 1, repel=TRUE) +
  ggtitle("Activities of Caracola del Este Corporation Express in 2030")
```
:::

```{r}
grouped <- MC2_edges %>% 
  group_by(source, Year, weightkg)
  sum(weightkg, valueofgoodsusd) %>% 
  ungroup() %>%
  na.omit() %>%
